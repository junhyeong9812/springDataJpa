...
//스프링 데이터 JPA 구현체 분석
//JPA리포지토리 중 SimpleJpaRepository가 존재
//이 구현체 내부를 보면
//findById/findAll등 기본 Jpa 함수를 구현하도록 설계되어 있다.
//쿼리 유틸 내부에 count QueryString도 별도로 가지고 있다.
//그래서 조회나 기본적인 동작은 Jpa 기능을 활용하는 것이며
//트랜잭션을 확인하면 리포지토리 어노테이션이 존재하고
//이게 존재하면 컴포넌트 스캔 대상이 되고 Jpa나 Jdbc의 영속성 계층 예외의 서로 다른 excep을 보고
//다 Spring에서 사용할 수 있는 예외로 변경해준다.
//그래서 컨트롤이나 서비스로 익셉션을 넘길 떄 Spring에서 제공하는 익셉션으로 변경해서 넘겨준다.
//그래서 Jpa에러도 익셉션 처리 방식은 컨트롤이나 서비스에서 처리하는게 동일하다.
//하부 구현을 변경되어도 내부 비즈니스 모델에는 변화가 거의 없다.
//@Transactional 또한 리드 온리 트루 설정이 되어 있다.
//그렇기 떄문에 서비스나 그런곳에서 트랜젝션을 열고 하면 리드 온지만 적용되지만
//별도의 트랜젝션을 걸지 않아도 동작할때 트렌잭션을 열고 함수가 동작하는 것을 알 수 있다.
//또한 Save같은 저장이 필요한 함수에는 기본 트랜젝션이 되어 읽기 전용이 아닌 저장이 가능한 트랜젝션이 걸린다.
//모든 JPA 변경은 트랜젝션 내부에서 일어나야 한다.
//springDataJpa를 사용하면 이렇게 함수가 동작할 때 트랜젝션이 동작해지는데
//트랜젝션이 없어도 동작이 되는데 만약 이렇게 하면 영속성 컨텍스트가 일치하지 않을 수 있기 떄문에 이부분을 신경써야 된다.
//또한 리드온리를 사용하게 되면
//jdbc의 메커니즘은 @트랜젝션을 걸면 DB커넥션에 오토커밋을 걸고 연결을 시켜주는데 이때
//트랜젝션 리드 온리를 하면 실제 트랜젝션 얻는 것과 일치하지만 이 리드온리 옵션을 통해 플러시를 생략해서 성능을 향상 시킬 수 있다.
//반대로 리드온리가 아니면 플래시 후 커밋을 한다는 것을 알 수 있다.
//그리고 이떄 리드온리는 더티체크 과정도 생략된다.

//또한 이때 Save함수에서는 새로운 함수일 경우에는 persist를 하지만
//기존의 엔티티라면 merge를 해버린다.
//이때 머지가 되면 기존의 데이터를 전부 날리고 이번에 들어온 엔티티 정보를 새롭게 저장(덮어쓰기)되어 버리기 때문에
//데이터가 날라갈 수 있다. 또한 db select를 통해 데이터가 존재하는 지 확인하는 쿼리가 나간다.
//그렇기 때문에 데이터 변경은 변경감지를 통해 해야 된다.
//Merge는 영속 상태를 벗어났을 때 다시 영속 상태로 만들기 위해서 사용하는 것
